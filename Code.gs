...
function doGet(e) {
  ...
  try {
    var result = _route('GET', action, params, null);
...
function doPost(e) {
  ...
  try {
    var result = _route('POST', action, e.parameter || {}, body);
    return _jsonResponse(result);
  } catch(err) {
...
function _route(method, action, params, body) {
  switch(action) {
    // ── Auth ──────────────────────────────────────────────────────
    ...
    case 'eliminarPedido':    return _eliminarPedido(body.id || params.id);
    ...
    case 'eliminarMotoboy':   return _eliminarMotoboy(body.idx !== undefined ? body.idx : params.idx);
    ...
    case 'eliminarUsuario':   return _eliminarUsuario(body.username || params.username);
    ...
    default:
      return { success: false, message: 'Acção desconhecida: "' + action + '"' };
  }
}
